...
import { StatusMessages, StatusMessage } from './StatusMessages.js';
...
// App Steps
type AppStep = 'selectModel' | 'validating' | 'findingImages' | 'processing' | 'summary' | 'error';

// Main App Component
export default function App() {
	const { exit } = useApp();
	const [step, setStep] = useState<AppStep>('selectModel');
	...
	const addMessage = (text: string) => {
		setStatusMessages((prev: StatusMessage[]) => [...prev, { 
			text,
	...
	const handleModelSelected = useCallback(async (selectedModel: ModelType) => {
		...
		try {
			...
			).map((file: string) => path.join('images', file)); // Keep the relative 'images/' path
	...
	useEffect(() => {
		...
		const processCurrentImage = async () => {
			...
			if (!imageFile) return; // Should not happen
			...
			try {
				...
				if (isActive) {
					...
					if (!apiError && !errorDetail) errorMessage += ` ${error.message}`;
	...
	return (
		<Box flexDirection="column" padding={1}>
			...
			{/* Removed 'idle' step rendering */}

			{/* Message display during validation and findingImages steps */}
			...
			{step === 'selectModel' && (
				<ModelSelector
					messages={statusMessages} // Pass empty initially
					onModelSelect={handleModelSelected}
				/>
			)}

			{/* ProcessingView */}
			{step === 'processing' && modelType && (
				<ProcessingView
					modelType={modelType}
			...
			{/* SummaryView - Rendered within a React Fragment */} 
			{step === 'summary' && (
				<>
